...
public interface TPropertyRepository extends JpaRepository<TProperty, Integer> {
    ...
    @Query("select t from TProperty t where t.title like concat('%', ?1,'%')")
    List<TProperty> findByTitleLike(String title, Pageable pageable);

    @Query("select t from TProperty t where t.title like concat('%', ?1,'%') and t.idCategory.id = ?2")
    ...
    @Query("select count(t) from TProperty t where t.title like concat('%', ?1,'%')")
    long countByTitleLike(String title);

    @Query("select count(t) from TProperty t where t.title like concat('%', ?1,'%') and t.idCategory.id = ?2")
